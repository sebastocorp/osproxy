...
package: check-go-target ## Package binary.
	...
	@if [ "$(OS)" = "linux" ]; then \
		tar --transform="s/$(BINARY)-$(GOOS)-$(GOARCH)/$(BINARY)/" -cvzf dist/$(PACKAGE_NAME) -C bin $(BINARY)-$(GOOS)-$(GOARCH) -C ../ LICENSE README.md; \
	elif [ "$(OS)" = "darwin" ]; then \
		tar -cvzf dist/$(PACKAGE_NAME) -s '/$(BINARY)-$(GOOS)-$(GOARCH)/$(BINARY)/' -C bin $(BINARY)-$(GOOS)-$(GOARCH) -C ../ LICENSE README.md; \
	else \
